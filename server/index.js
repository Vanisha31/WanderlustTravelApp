  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/packages') {
      ...
      const createdPackage = {
        ...
        destinations: Array.isArray(body.destinations) ? body.destinations : String(body.destinations).split(',').map((item) => item.trim()).filter(Boolean),
    ...
    const ticketMatch = url.pathname.match(/^\/api\/bookings\/([^/]+)\/ticket\.pdf$/);
    if (req.method === 'GET' && ticketMatch) {
      ...
      sendPdf(res, `${booking.id}-ticket.pdf`, createTicketPdf(booking));
      return;
    }

    if (req.method === 'POST' && url.pathname === '/api/ai/plan') {
      const body = await readBody(req);
      sendJson(res, 200, createAiPlan(body));
      return;
    }
    ...
    if (req.method === 'POST' && url.pathname === '/api/bookings') {
      ...
      const db = await readDb();
      const booking = {
        id: `WND-${Date.now().toString().slice(-6)}`,
        status: 'CONFIRMED',
        paymentStatus: 'PAID',
